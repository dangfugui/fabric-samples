...
func (t *State) Set(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) < 2 {
...
func (t *State) Get(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) < 1 {
		log.Warning("Incorrect arguments. Expecting a key and a value\n")
		return shim.Error("Incorrect arguments. Expecting a key and a value")
	}
...
// 通过弹珠ID查询所有的交易历史信息
func (t *State) MarbleHistoryForKey(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 1 {
